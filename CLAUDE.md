...
**Seed / maintenance scripts** (`docker/`, run via `wp eval-file`): `seed-products.php`, `seed-product-translations.php`, `seed-reviews.php`, `seed-about-text.php`, `seed-use-cases.php`, `fix-product-terms.php`.
...
- **`front-page.php`** — homepage, 7 sections in order: **1** Hero (customizer; falls back to bundled `assets/images/hero.jpg`), **2** Value proposition (static), **3** Best sellers (`best-seller` product tag), **4** Freeze-drying teaser (`sm_fd_image`), **5** About us (`sm_about_*`), **6** Reviews carousel (review comments, rating ≥ 4), **7** Instagram (`SM_Instagram_Feed`).
- Other templates: `page.php`, `page-sublimation.php` (freeze-drying education), `search.php` (products only), `404.php`, `sidebar.php`, `header.php`, `footer.php`, `index.php`.
...
| `myaccount/my-account.php` | Sidebar nav + content area — **inactive**, accounts are disabled (see `SM_Disable_Accounts`) |
...
- **`solmaram-core`** — loads on `plugins_loaded` (requires WooCommerce). Classes:
  - `SM_Free_Shipping_Bar` (hook + `[sm_shipping_bar]`), `SM_Instagram_Feed` (Graph API + transient), `SM_CSV_Export` (order CSV admin submenu), `SM_Trust_Counters` (`[sm_trust_counters]`), `SM_Admin_Settings` (options page).
  ...
  - **`SM_Disable_Blog`** — the editorial section is gone. Redirects the posts page, post permalinks and every post archive (category/tag/date/author) to the home page, catches the old Blog page slugs (`blog`, `blogue`, `блог`), and excludes posts from search. Genuine 404s still 404.
  - **`SM_Disable_Accounts`** — customer accounts are off. Redirects the My Account page and all its endpoints (any language) to the home page, forces guest checkout, disables registration (`users_can_register`, signup-from-checkout, login reminder), and strips My Account entries from front-end nav menus. Admin login and wp-admin are untouched. Remove the `SM_Disable_Accounts::init()` call in `solmaram-core.php` to re-enable accounts.
...
| FR-05 Accounts | **Disabled** — `SM_Disable_Accounts` (guest checkout only) |
| FR-06 Freeze-drying page | `page-sublimation.php` |
| FR-07 Reviews | WooCommerce built-in, `SM_Trust_Counters` shortcode |
| FR-08 Blog & Recipes | **Removed** — `SM_Disable_Blog` redirects all post routes to the home page |
